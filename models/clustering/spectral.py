...
class SpectralClustering(BaseClustering):
    ...
    def _compute_affinity_matrix(self, X: Tensor) -> Tensor:
        ...
        if self.affinity == 'rbf':
            ...
            _, indices = knn_graph(X, k=self.n_neighbors, metric=self.distance, p=self.p)
            
            affinity_matrix = torch.zeros((n_samples, n_samples), 
    ...
    def _compute_laplacian(self, affinity_matrix: Tensor) -> Tensor:
        ...
        if self.random_walk:
            ...
            laplacian = torch.eye(affinity_matrix.shape[0], device=self.device) - \
    ...
    def fit(self, X: Union[ndarray, Tensor]) -> 'SpectralClustering':
        ...
        if self.random_walk:
            ...
            embedding = eigenvectors[:, 1:self.n_clusters+1]  # Skip first trivial eigenvector
    ...
    def predict(self, X: Union[ndarray, Tensor]) -> Union[ndarray, Tensor]:
        ...
        for i in range(n_samples):
            if self.distance == "minkowski":
                distances = self.calculate_method(self.affinity_matrix_, X_tensor[i].unsqueeze(0), p=self.p)
            else:
                distances = self.calculate_method(self.affinity_matrix_, X_tensor[i].unsqueeze(0))
