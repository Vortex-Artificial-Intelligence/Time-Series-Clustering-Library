...
class HierarchicalClustering(BaseClustering):
    ...
    def _compute_distance_matrix(self, X: Tensor) -> Tensor:
        ...
        n_samples = X.shape[0]
        distance_matrix = torch.zeros((n_samples, n_samples), 
                                    device=self.device, dtype=self.dtype)
        
        for i in range(n_samples):
            for j in range(i + 1, n_samples):
                if self.distance == "minkowski":
                    dist = self.calculate_method(X[i].unsqueeze(0), X[j].unsqueeze(0), p=self.p)
                else:
    ...
    def _compute_ward_distance(self, cluster_i: Tuple, cluster_j: Tuple, 
                             cluster_info: Dict) -> float:
        ...
        if self.distance == "minkowski":
            dist = self.calculate_method(centroid_i.unsqueeze(0), centroid_j.unsqueeze(0), p=self.p)
        else:
            dist = self.calculate_method(centroid_i.unsqueeze(0), centroid_j.unsqueeze(0))
            
        ward_dist = torch.sqrt((2 * n_i * n_j * dist**2) / (n_i + n_j))
        return ward_dist.item()
    
    def _compute_linkage_distance(self, cluster_i: Tuple, cluster_j: Tuple,
                                distance_matrix: Tensor, cluster_info: Dict) -> float:
    ...
    def fit(self, X: Union[ndarray, Tensor]) -> 'HierarchicalClustering':
        ...
        for i in range(len(clusters)):
            for j in range(i + 1, len(clusters)):
                ...
                if self.linkage == 'ward':
                    dist = self._compute_ward_distance(cluster_i, cluster_j, cluster_info)
                else:
                    dist = self._compute_linkage_distance(cluster_i, cluster_j, distance_matrix, cluster_info)
        ...
        while len(clusters) > self.n_clusters:
            ...
            if self.linkage == 'ward':
                ...
                centroid_i = cluster_info[cluster_i]['centroid']
                centroid_j = cluster_info[cluster_j]['centroid']
                new_centroid = (size_i * centroid_i + size_j * centroid_j) / (size_i + size_j)
            else:
                new_centroid = None
            ...
            # Record the merge
            children.append((current_cluster_id, 
            ...
            for other_cluster in clusters:
                if other_cluster != new_cluster:
                    if self.linkage == 'ward':
                        dist = self._compute_ward_distance(new_cluster, other_cluster, cluster_info)
                    else:
                        dist = self._compute_linkage_distance(new_cluster, other_cluster, 
                                                            distance_matrix, cluster_info)
    ...
    def _compute_cluster_centers(self, X: Tensor) -> None:
        ...
        self.cluster_centers_ = torch.stack(cluster_centers) if cluster_centers else None
    ...
    def predict(self, X: Union[ndarray, Tensor]) -> Union[ndarray, Tensor]:
        ...
        if self.cluster_centers_ is None:
            raise ValueError("Cluster centers not available. Model may not be properly fitted.")
        ...
        for i in range(n_samples):
            if self.distance == "minkowski":
                distances = self.calculate_method(centroids, X_tensor[i].unsqueeze(0), p=self.p)
            else:
