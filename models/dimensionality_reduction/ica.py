...
class ICA(BaseDimensionalityReduction):
    ...
    def _fastica(self, X: Tensor) -> Tensor:
        ...
        W = torch.randn(self.n_components, self.n_components, device=self.device, dtype=self.dtype)
    ...
    def _infomax(self, X: Tensor) -> Tensor:
        ...
        for i in range(self.max_iter):
            ...
            # Compute gradient
            phi = 1 - 2 * y
            grad = torch.eye(self.n_components, device=self.device, dtype=self.dtype) + phi @ y.T / n_samples
    ...
    def fit(self, X: Union[ndarray, Tensor]) -> "ICA":
        ...
        if self.whiten:
            ...
            X_white = X_centered
            self.whitening_matrix_ = torch.eye(n_features, device=self.device, dtype=self.dtype)
