...
class MDS(BaseDimensionalityReduction):
    ...
    def _smacof_single(self, dissimilarities: Tensor, init: Tensor = None) -> Tuple[Tensor, float, int]:
        """SMACOF algorithm for a single run"""
        n_samples = dissimilarities.shape[0]
        
        # Initialize the embedding
        if init is None:
            X = torch.randn(n_samples, self.n_components, device=self.device, dtype=self.dtype)
        else:
            X = init.clone()
    ...
    def fit(self, X: Union[ndarray, Tensor]) -> "MDS":
        ...
        self.dissimilarity_matrix_ = (self.dissimilarity_matrix_ + self.dissimilarity_matrix_.T) / 2
        ...
        for i in range(self.n_init):
            ...
            init = torch.randn(n_samples, self.n_components, device=self.device, dtype=self.dtype)
            
            # Run SMACOF
            embedding, stress, n_iter = self._smacof_single(self.dissimilarity_matrix_, init)
    ...
    def transform(self, X: Union[ndarray, Tensor]) -> Union[ndarray, Tensor]:
        ...
        raise NotImplementedError("MDS does not support transforming new data. Use fit_transform instead.")
