...
class TSNE(BaseDimensionalityReduction):
    ...
    def __init__(
        ...
        if self.method not in ['exact', 'barnes_hut']:
            raise ValueError(f"method must be 'exact' or 'barnes_hut', got {self.method}")
        
        if self.metric not in ['euclidean', 'manhattan', 'chebyshev', 'minkowski', 'cosine']:
            raise ValueError(f"metric must be 'euclidean', 'manhattan', 'chebyshev', 'minkowski', or 'cosine', got {self.metric}")
    ...
    def _barnes_hut_tsne_grad(self, P: Tensor, Y: Tensor) -> Tuple[Tensor, float]:
        ...
        if self.n_components == 2:
            ...
            grid_cells = torch.linspace(min_coords[0], max_coords[0], grid_size + 1, device=self.device)
            grid_cells_y = torch.linspace(min_coords[1], max_coords[1], grid_size + 1, device=self.device)
            
            # Compute cell centers and masses
            cell_centers = torch.zeros((grid_size, grid_size, 2), device=self.device, dtype=self.dtype)
            cell_masses = torch.zeros((grid_size, grid_size), device=self.device, dtype=self.dtype)
            ...
            for i in range(grid_size):
                for j in range(grid_size):
                    ...
                    in_cell = (Y[:, 0] >= x_min) & (Y[:, 0] < x_max) & (Y[:, 1] >= y_min) & (Y[:, 1] < y_max)
            ...
            for i in range(n):
                ...
                for cell_i in range(grid_size):
                    for cell_j in range(grid_size):
                        ...
                        cell_size = max(grid_cells[1] - grid_cells[0], grid_cells_y[1] - grid_cells_y[0])
    ...
    def fit(self, X: Union[ndarray, Tensor]) -> "TSNE":
        ...
        if self.method == 'barnes_hut' and self.n_components not in [2, 3]:
            if self.verbose:
                print("Barnes-Hut is only applicable to 2D or 3D embeddings. Switching to exact method.")
            self.method = 'exact'
        ...
        if isinstance(self.init, Tensor):
            ...
            Y = torch.randn(n_samples, self.n_components, device=self.device, dtype=self.dtype)
        ...
        for i in range(self.n_iter):
            ...
            # Update gains
            gains = (gains + 0.2) * ((grad > 0) != (iY > 0)).float() + \
                    (gains * 0.8) * ((grad > 0) == (iY > 0)).float()
    ...
    def transform(self, X: Union[ndarray, Tensor]) -> Union[ndarray, Tensor]:
        ...
        raise NotImplementedError("TSNE does not support transforming new data. Use fit_transform instead.")
