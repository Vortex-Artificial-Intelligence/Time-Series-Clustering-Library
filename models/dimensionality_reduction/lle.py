...
class LLE(BaseDimensionalityReduction):
    ...
    def __init__(
        ...
        if self.method not in ['standard', 'hessian', 'modified', 'ltsa']:
            raise ValueError(f"method must be 'standard', 'hessian', 'modified', or 'ltsa', got {self.method}")
        ...
        for i in range(n_components):
            ...
            for _ in range(max_iter):
                ...
                try:
                    ...
                    L = torch.linalg.cholesky(A - sigma * torch.eye(n, device=self.device, dtype=self.dtype))
                    b_new = torch.cholesky_solve(b.unsqueeze(1), L).squeeze()
                except:
                    # Fall back to LU decomposition
                    b_new = torch.linalg.solve(A - sigma * torch.eye(n, device=self.device, dtype=self.dtype), b)
    ...
    def _compute_weights_standard(self, X: Tensor, indices: Tensor) -> Tensor:
        ...
        for i in range(n_samples):
            ...
            try:
                ...
                w = torch.cholesky_solve(torch.ones(n_neighbors, device=self.device, dtype=self.dtype), L)
            except:
                # Fall back to pseudo-inverse
                w = torch.linalg.pinv(C) @ torch.ones(n_neighbors, device=self.device, dtype=self.dtype)
    ...
    def _compute_weights_hessian(self, X: Tensor, indices: Tensor) -> Tensor:
        ...
        for i in range(n_samples):
            ...
            H = torch.zeros((d * (d + 1) // 2, n_neighbors), device=self.device, dtype=self.dtype)
    ...
    def _compute_embedding(self, W: Tensor, method: str) -> Tensor:
        ...
        W_sparse = torch.zeros((n_samples, n_samples), device=self.device, dtype=self.dtype)
        
        # Create indices for the sparse matrix
        row_indices = torch.arange(n_samples, device=self.device).repeat_interleave(n_neighbors)
        col_indices = torch.arange(n_samples, device=self.device).repeat(n_neighbors).reshape(n_neighbors, n_samples).T.reshape(-1)
    ...
    def fit(self, X: Union[ndarray, Tensor]) -> "LLE":
        ...
        self.reconstruction_error_ = torch.norm(X - self.embedding_ @ self.embedding_.T @ X)
    ...
    def transform(self, X: Union[ndarray, Tensor]) -> Union[ndarray, Tensor]:
        ...
        raise NotImplementedError("LLE does not support transforming new data. Use fit_transform instead.")
