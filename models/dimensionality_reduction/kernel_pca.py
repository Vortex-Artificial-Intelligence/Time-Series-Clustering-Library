...
class KernelPCA(BaseDimensionalityReduction):
    ...
    def _get_kernel(self, kernel):
        ...
        if callable(kernel):
            ...
            return lambda X, Y: polynomial_kernel(X, Y, degree=self.degree, gamma=self.gamma, coef0=self.coef0)
        elif kernel == "rbf":
    ...
    def fit(self, X: Union[ndarray, Tensor]) -> "KernelPCA":
        ...
        n_samples = X.shape[0]
        one_n = torch.ones((n_samples, n_samples), dtype=self.dtype, device=self.device) / n_samples
        K_centered = K - one_n @ K - K @ one_n + one_n @ K @ one_n
        
        # Add small regularization for numerical stability
        K_centered += torch.eye(n_samples, dtype=self.dtype, device=self.device) * self.alpha
    ...
    def transform(self, X: Union[ndarray, Tensor]) -> Union[ndarray, Tensor]:
        ...
        one_n_train = torch.ones((n_samples_train, n_samples_train), dtype=self.dtype, device=self.device) / n_samples_train
        one_n_test = torch.ones((n_samples_test, n_samples_train), dtype=self.dtype, device=self.device) / n_samples_train
        
        K_centered = K - one_n_test @ kernel_fn(self.X_fit_, self.X_fit_) - K @ one_n_train + one_n_test @ kernel_fn(self.X_fit_, self.X_fit_) @ one_n_train
